...
export default function ProductShowcase() {
    ...
    const triggerTransition = (nextIndex: number) => {
        ...
        // Burst particles appear immediately
        setTimeout(() => {
    ...
    return (
        <section className="relative min-h-screen overflow-hidden bg-[#0a0a0a]">
            {/* Dynamic Background */}
            <div className={`absolute inset-0 bg-gradient-to-br ${current.bgGradient} transition-all duration-1000`} />
            ...
            <div className="relative z-10 min-h-screen flex items-center px-8 md:px-16 lg:px-24">
                <div className="w-full max-w-7xl mx-auto grid lg:grid-cols-2 gap-8 lg:gap-16 items-center py-24">
                    ...
                    {/* Left - Typography with explosive entrance */}
                    <div className={`text-white space-y-6 showcase-text-enter ${isAnimating ? 'animating' : ''}`}
                         style={{ '--text-dir': direction === 'next' ? '1' : '-1' } as React.CSSProperties}>
                        {current.badge && (
                            <span className="inline-block px-4 py-2 text-xs font-bold uppercase tracking-widest rounded-full"
                                  style={{
                        ...
                        <div className="pt-4">
                            <a href="/shop"
                               ...
                               style={{ background: current.accentColor, boxShadow: `0 0 40px ${current.accentColor}44` }}>
                                <span>Shop Now</span>
                    ...
                    {/* Right - Product Image with EXPLOSIVE animations */}
                    <div className="relative flex justify-center items-center" style={{ perspective: '800px' }}>
                        ...
                        <div className="absolute w-72 h-72 md:w-96 md:h-96 lg:w-[450px] lg:h-[450px] rounded-full opacity-30 transition-colors duration-1000"
                             style={{ borderColor: current.accentColor, border: `2px solid ${current.accentColor}50` }}>
                            <div className="showcase-revolve-ring" style={{ borderColor: current.accentColor }} />
                        </div>

                        {/* Revolving ring 2 - inner, reverse */}
                        <div className="absolute w-56 h-56 md:w-72 md:h-72 lg:w-80 lg:h-80 rounded-full opacity-20">
                                ...
                                {[...Array(20)].map((_, i) => (
                                    <div key={`burst-${currentSlide}-${i}`} className="showcase-burst-particle"
                                         style={{
                                             ...
                                             background: i % 2 === 0 ? current.accentColor : '#ffffff',
                        ...
                        <div className={`relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 z-10 showcase-product-enter ${isAnimating ? 'animating' : ''}`}
                             style={{
                                 ...
                                 filter: `drop-shadow(0 0 80px ${current.accentColor}88)`,
            ...
            <button onClick={handlePrev}
                    className="absolute left-8 md:left-12 top-1/2 -translate-y-1/2 z-30 w-12 h-12 flex items-center justify-center bg-white/10 backdrop-blur-sm rounded-full hover:bg-white/20 hover:scale-110 transition-all duration-300"
            ...
            <button onClick={handleNext}
                    className="absolute right-8 md:right-12 top-1/2 -translate-y-1/2 z-30 w-12 h-12 flex items-center justify-center bg-white/10 backdrop-blur-sm rounded-full hover:bg-white/20 hover:scale-110 transition-all duration-300"
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex gap-3">
                {products.map((_, index) => (
                    <button key={index} onClick={() => goToSlide(index)}
                            className={`h-3 rounded-full transition-all duration-500 ${index === currentSlide ? 'w-8 bg-white' : 'w-3 bg-white/40 hover:bg-white/60'}`}
